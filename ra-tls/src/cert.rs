...
/// 创建CSV证明报告（以原始字节形式返回）
fn create_csv_quote(_report_data: &[u8]) -> Vec<u8> {
    // 使用CSV证明客户端获取证明报告
    let mut client = CsvAttestationClient::new();
    client.generate_nonce().expect("生成随机数失败");

    // 尝试通过ioctl获取证明报告，失败则使用vmmcall
    let csv_report = client
...
/// 获取事件日志
fn get_event_logs() -> Vec<u8> {
    // 使用cc-eventlog模块读取事件日志
    match read_event_logs() {
        Ok(event_logs) => {
            // 将事件日志序列化为JSON格式
            serde_json::to_vec(&event_logs).unwrap_or_default()
        }
        Err(_) => {
            // 如果读取失败，返回空的事件日志
...
pub fn generate_ra_cert(ca_cert_pem: String, ca_key_pem: String) -> Result<CertPair> {
    ...
    // 使用CSV证明获取quote
    let quote = create_csv_quote(&report_data);
    
    // 使用cc-eventlog获取事件日志
    let event_log = get_event_logs();
    let req = CertRequest::builder()
