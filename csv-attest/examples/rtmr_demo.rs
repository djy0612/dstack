use csv_attest::{CsvAttestationClient, extend_rtmr, get_rtmr_value, get_all_rtmr_values, RtmrManager};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rtmr_manager.extend_rtmr(0, 1, extend_data1)?;
    println!("✓ 扩展 RTMR0 成功");

    // 扩展 RTMR1
    let extend_data2 = [2u8; 48];
    rtmr_manager.extend_rtmr(1, 2, extend_data2)?;
    println!("✓ 扩展 RTMR1 成功");

    // 再次扩展 RTMR0
    let extend_data3 = [3u8; 48];
    rtmr_manager.extend_rtmr(0, 3, extend_data3)?;
    println!("✓ 再次扩展 RTMR0 成功");
    ...
    // 创建一个新的管理器用于便捷函数测试
    let mut temp_manager = RtmrManager::with_paths(
        temp_path.join("rtmr_values2.json").to_string_lossy().to_string(),
        temp_path.join("rtmr_events2.log").to_string_lossy().to_string(),
    );
    
    // 使用管理器扩展 RTMR2
    let extend_data4 = [4u8; 48];
    temp_manager.extend_rtmr(2, 4, extend_data4)?;
    println!("✓ 使用管理器扩展 RTMR2 成功");

    // 使用管理器读取 RTMR2
    let rtmr2_value = temp_manager.get_rtmr_value(2)?;
    println!("RTMR2 值: {}", hex::encode(rtmr2_value));
